...
function nodeExplorer(targetNode) {
    for (var node = targetNode.firstChild; node; node = node.nextSibling) {
        if (! node.data) {
            this.push(node.nodeName.toLowerCase());
        }

        if (node.firstChild) {
            nodeExplorer.call(this, node);
        }
    }
}
...
var svgPrune = function(data) {
    ...
    /*
        TODO: _.without list for skipped tags should be extended 
        when we add a support for conversion other tags to path
    **/
    ...
    for (var i=0; i<pathTags.length; i++) {
        ...
        var pathAttrsSkip = _.without(_.map( node.attributes, function(attr) { return attr.nodeName.toLowerCase(); }), 'transform', 'd');
        ...
        **/
        ...
        for (var group = node.parentNode; group.nodeName == 'g'; group = group.parentNode) {
            var groupAttrsSkip = _.without(_.map(group.attributes, function(attr) { return attr.nodeName.toLowerCase(); }), 'transform');
            ...
            if (groupTransform) {
                path = new SvgPath(path).transform(groupTransform).toString();
            }
        }
