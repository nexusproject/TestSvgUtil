...
var svgPrune = function(data) {
  ...
  for (var i=0; i<pathTags.length; i++) {
    ...
    var pathAttrsSkip = _.intersection(disallowedSvgAttributes, _.map( node.attributes, function(attr) { return attr.nodeName.toLowerCase(); }));
    ...
    **/
    ...
    for (var group = node.parentNode; group.nodeName == 'g'; group = group.parentNode) {
      ...
      var groupAttrsSkip = _.intersection(disallowedSvgAttributes, _.map(group.attributes, function(attr) { return attr.nodeName.toLowerCase(); }));
